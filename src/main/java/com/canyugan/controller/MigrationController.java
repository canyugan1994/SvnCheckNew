			...
			Future<String> lx_thread_result = pool.submit(new Callable<String>()
			{
				...
				public String call() throws Exception 
				{
					...
					try {
						...
						for(String project_key:project_keys)//遍历勾选的项目
						{
							if(!project_key.contains("LX")) {//仅迁移lx
								continue;
							}
							...
							for (String depart : depart_url) 
							{
								...
								for(String rqn:rqn_year)
								{
									if (rqn.contains("RQN"))//有的目录不是以RQN开头 忽略 加快迁移速度
									{
										...
										for(Map<String, String> temp_map_single:temp_map_list)//遍历每个项目下面的所以RQN编号
										{
											temp_map = temp_map_single;
											if(temp_map.get("project_num").equals(rqn_direct))//项目的此rqn编号和svn的rqn编号能匹配上
											{
												...
												for(String rqn_dir:rqn_dirs) //01-需求 || 02-TF目录
												{
													...
													if(rqn_dir_direct.contains("TF") || rqn_dir_direct.contains("需求"))//02-TF
													{
														...
														{
															//匹配立项类项目的项目名和要迁移的项目 
															String lx_project_url_direct = 
																	lx_project_url.substring(lx_project_url.lastIndexOf("/")+1,lx_project_url.length());
															if(lx_project_url_direct.equals(project_key)) //PJ-LX-2018-002-001-人工智能平台运营
															{
																LOG.info("-->立项类项目迁移线程【 svn项目目录和"
																		+ "待迁移的项目一致 ["+lx_project_url_direct+"]=["+project_key+"] 】");
																...
																for(String lx_project_urls_other_single:lx_project_urls_other) //01项目管理文档 02项目文档 03其它文档
																{
																	...
																	String other_direct = lx_project_urls_other_single.substring(
																			lx_project_urls_other_single.lastIndexOf("/")+1,
																			lx_project_urls_other_single.length());
																	if(other_direct.contains("项目文档")) 
																	{
																		List<String> find_tf_dirs = getSvnBaseInfo(
																						dist_repository,
																						"立项类项目/"+lx_project_url_direct+"/"+other_direct,
																						"dir","dist"); 
																		for(String find_tf_dir:find_tf_dirs)//02需求 03TF
																		{
																			String tf_dir = 
																					find_tf_dir.substring(find_tf_dir.lastIndexOf("/")+1, find_tf_dir.length());
			...
			Future<String> wh_thread_result = pool.submit(new Callable<String>()
			{
				...
				public String call() throws Exception 
				{
					...
					try 
					{
						...
						{
							if(!project_key.contains("WH")) {//仅仅迁移lx
								continue;
							}
							String year = dateUtil.currentDateTime().substring(0, 4);//获取当前执行年份
							List<String> depart_url = getSvnBaseInfo(need_repository, "","dir","need");//获取svn根路径下的部门路径
							for (String depart : depart_url) 
							{
								...
								for(String rqn:rqn_year) 
								{
									if (rqn.contains("RQN"))//有的目录不是以RQN开头 忽略 加快迁移速度
									{
										...
										{
											temp_map = temp_map_single;
											if(temp_map.get("project_num").equals(rqn_direct))//项目的rqn编号和svn的rqn编号能匹配上 
											{
												LOG.info("-->维护类项目迁移线程【 svn上的RQN编号和项目的RQN编号匹配成功"
														+ "RQN["+rqn_direct+"] projectName["+project_key+"] 】");
												List<String> rqn_dirs = 
														getSvnBaseInfo(need_repository,depart+"/"+year+"/"+rqn_direct,"dir","need");
												for(String rqn_dir:rqn_dirs) //01-需求 || 02-TF目录
												{
													String rqn_dir_direct = 
															rqn_dir.substring(rqn_dir.lastIndexOf("/"), rqn_dir.length());
													/**
													 * step 2: 目标库
													 */
													List<String> wh_project_urls = 
															getSvnBaseInfo(dist_repository, "维护优化项目","dir","dist");
													if(rqn_dir_direct.contains("TF") || rqn_dir_direct.contains("需求"))//02-TF
													{
			...
			pool.submit(new Callable<String>()
			{
				...
				public String call() throws Exception 
				{
					...
					if(message_lx.equals("success") && message_wh.equals("success")) {
						...
						LOG.info("-->【 需求库立项类项目迁移失败 数据库记录失败标记 】");
					}else if(message_wh.equals("error")) {
						LOG.info("-->【 需求库维护类迁移失败 数据库记录失败记录 】");
					}
					//失败
