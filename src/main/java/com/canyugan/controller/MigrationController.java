...
public class MigrationController 
{
	...
	private DateUtil dateUtil = new DateUtil();//时间工具类
...
//																				//匹配到目标库项目对应的tf目录
//																				String path = 
//																						svn_save_path+rqn_direct+new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
...
//														{
//															//匹配维护类项目的项目名和要迁移的项目
//															String wh_project_url_direct = 
//																	wh_project_url.substring(wh_project_url.lastIndexOf("/")+1,wh_project_url.length());
...
//																		dist_commitClient.doMkDir(new SVNURL[] {
//																				SVNURL.parseURIEncoded(
//																						dist_svn_url+"/维护优化项目/"+project_key+
//																						"/"+project_Version+"/"+project_num+"/"+child_dir)
//																		    },null);
//																	}
...
//																									        project_key+"/"+project_Version+"/"+
//																											project_num+"/"+child_dir+"/"+
//																									        single_file.getName()),null,false);
...
//																		}
//																	}else {
//																		LOG.info("-->维护类项目迁移线程【 在服务器文件系统上创建目录[" + path + "]失败】");
//																	}
//																} catch (Exception e) {
...
//			});
			...
			Future<String> tc_thread = pool.submit(new Callable<String>()
			{
				...
				public void list(SVNRepository repository, String path, List<String> result,String type,String source) {
					try {
						...
						LOG.info("-->TC文档迁移线程【 获取目录下面的目录或者文件[" + path + "]失败，失败信息：" + e.getMessage() + " 】");
					}
				}
				...
				public String call() throws Exception 
				{
					 ...
					 try {
						...
						LOG.info("-->TC文档迁移线程【 svn服务器连接失败，失败信息["+e.getMessage()+"] 】");
						return "error";
					 }
					 try {
						File first_path = new File(save_path);//读取tc文档所在的存储根目录 迁移的发源地
						if(!first_path.exists()) {//如果根目录被删
							LOG.info("-->TC文件系统库【 TC文档所在的根目录["+first_path.getName()+"]不存在 请检查目录是否存在 TC迁移作废 不再继续进行TC迁移 】");
							return "error";//快速失败
						}
						Set<String> project_keys = all_project_needMove.keySet();//待迁移项目的集合 来源于双速需求接口提供的信息
						Set<Map<String, String>> temp_map_list = new HashSet<Map<String,String>>();//指定项目对应的RQN集合 一次性
						SVNCommitClient dist_commitClient = dist_clientManager.getCommitClient();//目标库提交文档、创建目录客户端 
						for(File depart_file:first_path.listFiles()) //遍历磁盘上的部门目录
						{
							String departName = depart_file.getName();//获取部门名称
							LOG.info("-->TC文件系统库【 正在处理部门["+departName+"] 】");
							File second_path = new File(save_path + "/" + departName);//获取部门下面的目录
							if(!second_path.exists()) {//如果目录在文件列出后被删
								LOG.info("-->TC文件系统库【 TC文档所在的部门目录[" + second_path.getName() + "]不存在 】");
								continue;
							}
							//由于一个项目可以对应多个修复的版本 故把项目的遍历放在外面
							for(String project_key:project_keys)//遍历待迁移项目的key
							{
								LOG.info("-->TC文件系统库【 正在处理项目["+project_key+"] 对应部门["+departName+"] 】");
								for(File release_version_file:second_path.listFiles()) //遍历该部门下的修复的版本目录
								{
									String releaseVsesion = release_version_file.getName();//获取修复的版本
									LOG.info("-->TC文件系统库【 正在处理修复的版本["+releaseVsesion+"] "
											+ "对应项目["+project_key+"] "
													+ "对应部门["+departName+"] 】");
									File third_path = new File(save_path + "/" + departName + "/" + releaseVsesion);//获取修复的版本下面的需求编号
									if(!third_path.exists()) {//如果目录在文件列出后被删
										LOG.info("-->TC文件系统库【 TC文档所在的修复版本目录[" + third_path.getName() + "]不存在 】");
										continue;
									}
									for(File current_rqn:third_path.listFiles())//遍历修复的版本下面的RQN编号list
									{
										String current_rqn_code = current_rqn.getName();//获取rqn编号
										LOG.info("-->TC文件系统库【 正在处理RQN编号["+current_rqn_code+"] "
												+ "修复的版本["+release_version_file+"] "
														+ "对应项目["+project_key+"] "
																+ "对应部门["+departName+"] 】");
										File fourthly_path = 
												new File(save_path + "/" + departName + "/" + releaseVsesion + "/" + current_rqn_code);
										if(!fourthly_path.exists()) {//如果目录在文件列出后被删
											LOG.info("-->TC文件系统库【 TC文档所在的需求编号目录[" + fourthly_path.getName() + "]不存在 】");
											continue;
										}
										temp_map_list = all_project_needMove.get(project_key);//获取指定项目的需求编号和修复的版本信息
										for(Map<String, String> project_map:temp_map_list)//磁盘上的修复的版本目录名和该项目的修复的版本名对比
										{
											if(!project_map.get("projectVersion").contains("Release")) {
												continue;//如果修复的版本是【非常规发布】，忽略
											}
											String version = null;
											String release =null;
											try {
												version = project_map.get("projectVersion");//Release-20190806
												//version = version.split("-")[1];//获取时间
												//release = releaseVsesion.substring(0,10);//2019-08-06 13:45:46.1
												//release = release.substring(0,10).replace("-","");//去除-
											} catch (Exception e) {
												LOG.info("-->TC文件系统库【 处理接口修复的版本字段和tc的date目录失败，失败信息["+e.getMessage()+"] 】");
												continue;
											}
											LOG.info("-->TC文件系统库【 项目的修复版本["+version+"] 文件系统修复的版本["+releaseVsesion+"] 】");
											//对应成功
											//if(version != null && release != null && version.equals(releaseVsesion)) 
											if(version != null  && version.equals(releaseVsesion)) 
											{
												LOG.info("-->TC文件系统库【 项目["+project_key+"]匹配成功 】");
												if(project_key.contains("LX"))//如果项目编号带LX 属于立项类项目
												{
													LOG.info("-->TC文件系统库【 项目["+project_key+"]属于立项类项目 】");
													try 
													{
														List<String> lx_project_urls = //获取该立项类项目的下级目录
																getSvnBaseInfo(dist_repository, "立项类项目/"+project_key,"dir","dist");
														LOG.info("-->TC文件系统库【 项目["+project_key+"]从svn拉取目录成功 】");
														boolean project_doc_exist = false;
														for(String url:lx_project_urls)
														{
														...
														if(!project_doc_exist) {//不存在02 项目文档则帮忙创建
															dist_commitClient.doMkDir(new SVNURL[] {
																	SVNURL.parseURIEncoded(
																			dist_svn_url+"/立项类项目/"+project_key+"/"+url)
															    },null);
														}
														List<String> lx_project_urls_final = //获取02 项目目录的下级目录
																getSvnBaseInfo(dist_repository, "立项类项目/"+project_key+"/"+url,"dir","dist");
														LOG.info("-->TC文件系统库【 项目["+project_key+"]从svn拉取[项目文档]目录成功 】");
														//判断06-测试目录存在与否
														...
														if(!test_exist) {//如果不存在06 测试沐流 则创建
															dist_commitClient.doMkDir(new SVNURL[] {
																	SVNURL.parseURIEncoded(
																			dist_svn_url+"/立项类项目/"+project_key+"/"+url+"/"+"06 测试")
															    },null);
															LOG.info("-->TC文件系统库【 项目["+project_key+"]在svn创建[06 测试]目录成功 】");
														}
														LOG.info("-->TC文件系统库【 项目["+project_key+"]开始从tc迁移到svn目标库立项类项目 】");
											    		//迁移单位：单个文件
														for(File single_file:fourthly_path.listFiles())
														{
															if(single_file.isFile())//避免迁移恶意创建的目录
															{
												    			try {//迁移每个文档到目标库的测试下
												    				LOG.info("-->TC文件系统库【 迁移到的目录[" + 
												    						dist_svn_url+"/立项类项目/"+
												    						project_key+"/"+url+"/06 测试]" + " 】");
																	dist_commitClient.doImport(
																			single_file,//文件在磁盘文件的路径
																			SVNURL.parseURIEncoded(
																					dist_svn_url+"/立项类项目/"+
																					...
																					single_file.getName()),
																					null,false);
																	LOG.info("-->TC文件系统库【 文档["+single_file+"]迁移到目标库立项类06 测试目录迁移完成】");
														    	} catch (Exception e) {
																	LOG.info("-->TC文件系统库【 上传文档到svn目标库立项类06 测试目录，失败信息["+e.getMessage()+"] ");
																}
												    		}
														}
													} catch (Exception e) {
														LOG.info("-->TC文件系统库【 迁移文档到目标库失败，失败信息[" + e.getMessage() + "] 】");
													}
												}else if(project_key.contains("WH")) {//维护类项目 迁移到目标库维护类项目测试目录
													LOG.info("-->TC文件系统库【 项目["+project_key+"]属于维护类项目 】");
													List<String> wh_project_urls = 
															getSvnBaseInfo(dist_repository, "维护优化项目/"+project_key,"dir","dist");
													LOG.info("-->TC文件系统库【 项目["+project_key+"]从svn拉取目录成功 修复的版本["+wh_project_urls+"] 】");
													//？？难道是空的？
													for(String release_url:wh_project_urls)//遍历该项目下的修复的版本 
													{
														try {
															String current_project_rqncode = project_map.get("project_num");//获取待迁移项目的rqn编号
															if(release_url.equals(project_map.get("projectVersion")))//如果和当前迁移项目的修复版本匹配 进行迁移
															{
																LOG.info("-->TC文件系统库【 项目["+project_key+"]的"
																		+ "修复版本["+project_map.get("projectVersion")+"]和在"
																				+ "svn目录上修复的版本["+release_url+"]匹配成功 】");
																List<String> wh_project_rqn_urls = 
																		getSvnBaseInfo(dist_repository, "维护优化项目/"+project_key+"/"+release_url,"dir","dist");
																LOG.info("-->TC文件系统库【 获取项目["+project_key+"]在svn目录上的修复的版本的下属层级 】");
																boolean rqn_exist = false;
																for(String rqn_url:wh_project_rqn_urls) {
																	if(rqn_url.equals(current_project_rqncode)) {
																		rqn_exist = true;
																		break;
																	}
																}
																if(!rqn_exist) {//如果rqn编号不存在 直接帮忙创建
																	dist_commitClient.doMkDir(new SVNURL[] {
																			SVNURL.parseURIEncoded(dist_svn_url+
																					"/维护优化项目/"+project_key+"/"+release_url+"/"+current_project_rqncode)
																	    },null);
																	LOG.info("-->TC文件系统库【 项目["+project_key+"]在svn目录上RQN编号不存在 创建 】");
																}
																//存在即获取03-测试目录 不存在创建
																List<String> wh_project_rqn_final_urls = 
																		getSvnBaseInfo(dist_repository, 
																				"维护优化项目/"+project_key+"/"+release_url+"/"+current_project_rqncode,"dir","dist");
																LOG.info("-->TC文件系统库【 获取项目["+project_key+"]在svn目录上的[03-测试]目录 】");
																boolean test_exist = false;
																for(String test_url:wh_project_rqn_final_urls) {
																	if(test_url.contains("测试")) {
																		test_exist = true;
																		break;
																	}
																}
																if(!test_exist) {//如果03-测试目录不存在 直接帮忙创建
																	dist_commitClient.doMkDir(new SVNURL[] {
																			SVNURL.parseURIEncoded(dist_svn_url+
																					"/维护优化项目/"+project_key+"/"+release_url+"/"+current_project_rqncode+"/03-测试")
																	    },null);
																	LOG.info("-->TC文件系统库【 项目["+project_key+"]在svn目录上的[03-测试]目录不存在 创建 】");
																}
																/**至此 维护类项目的测试完整路径[项目编号+项目名/修复的版本/需求编号/03-测试]已存在
																 *可以开始把tc目录的文档迁移过来
																 */
																LOG.info("-->TC文件系统库【 项目["+project_key+"]开始从tc迁移到svn目标库维护类项目 】");
																//迁移单位：单个文件
																for(File single_file:fourthly_path.listFiles())
																{
																	if(single_file.isFile())//排除恶意的人创建目录 做无谓迁移
																	{
														    			try {//迁移每个文档到目标库的测试下
														    				LOG.info("-->TC文件系统库【 迁移到的目录[" + 
														    						dist_svn_url+"/维护优化项目/"+
														    						project_key+"/"+release_url+"/"+
														    						current_project_rqncode+"/03-测试"+"] 】");
																			dist_commitClient.doImport(
																					single_file,//文件在磁盘文件的路径
																					SVNURL.parseURIEncoded(
																							dist_svn_url+"/立项类项目/"+
																							...
																							single_file.getName()),
																					null,false);
																			LOG.info("-->TC文件系统库【 文档["+single_file+"]到目标库维护类目录迁移完成】");
																    	} catch (Exception e) {
																			LOG.info("-->TC文件系统库【 迁移文档到目标库维护类03-测试目录失败,失败信息:"+e.getMessage()+ " 】");
																		}
														    		}
																}
																break;
															...
															LOG.info("-->TC文件系统库【 迁移文档到目标库维护类目录失败,失败信息:" + e.getMessage() +  " 】");
														}
													}
												}
												break;//该rqn编号找到了对应的项目并且迁移到目标库就就退出循环
											}
										}
									}
								}
			...
			pool.submit(new Callable<String>()
			{
				...
				public String call() throws Exception 
				{
					...
					String message_tc = tc_thread.get();
					//if(message_lx.equals("success") && message_wh.equals("success") && message_tc.equals("success")) {
					if(message_tc.equals("success")) {
						LOG.info("-->【 立项类、维护类迁移成功 】");
						//成功
						for(String project_key:all_project_needMove.keySet()) {//设置成功标记和迁移成功的时间
							svnCheckService.updateProjectMoveStatusAndDate(project_key, "2", dateUtil.currentDateTime());
						}
						pool.shutdown();//关闭线程池
...
//						LOG.info("-->【 需求库立项类项目迁移失败 】");
//					}
//					else if(message_wh.equals("error")) {
//						LOG.info("-->【 需求库维护类迁移失败 】");
//					}
					else if(message_tc.equals("error")) {
						LOG.info("-->【 TC文档迁移目标库失败 】");
					}
					//失败
					for(String project_key:all_project_needMove.keySet()) {//设置失败标记
						svnCheckService.updateProjectMoveStatusAndDate(project_key, "3",null);
					}
					return "error";
				}
			});
			//迁移中
			for(String project_key:all_project_needMove.keySet()) {//设置标记
				svnCheckService.updateProjectMoveStatusAndDate(project_key, "1",null);
			}
			result.put("status","success");
			//0待迁移 1迁移中 2迁移完成 3迁移失败
			result.put("message","迁移任务提交成功 后续可以刷新当前页面进行迁移进度查询");
			return result;
		}catch (Exception e) {
